...
class ExistButDeletedDashboard implements Rule
{
    private $model;
    private $adminId;
    ...
    public function passes($attribute, $value)
    {
        if ($attribute == 'phone') {
            ...
            if (!$itemDoesntExist) {
                ...
                $itemDoesnt = $itemDoesntExist;
            } elseif (!$existsNormalized) {
                $this->model = $this->model->onlyTrashed()->where($attribute, $normalizedValue)->first();
                $itemDoesnt = $existsNormalized;
            }
            return $itemDoesnt;
        } else {
            $itemDoesntExist = $this->model->onlyTrashed()->where($attribute, $value)->doesntExist();
